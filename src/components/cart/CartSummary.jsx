/**
 * CartSummary.jsx
 * Order totals, coupon input, and free-shipping progress bar.
 * Used in both CartDrawer and cart/page.jsx.
 */
...
export function CartSummary({ onCheckout, compact = false }) {
  ...
  const handleApplyCoupon = async () => {
    ...
    // Mock coupon validation (matches useCart.js mock data)
    const MOCK_COUPONS = {
      WELCOME10: { type: 'percentage',    value: 10,  label: '10% off' },
      SAVE50:    { type: 'fixed',         value: 50,  label: 'KES 50 off' },
    ...
    if (coupon) {
      ...
      setCouponMsg(`"${code}" applied — ${coupon.label}`);
      setCouponInput('');
    } else {
      setCouponStatus('error');
      setCouponMsg('Invalid coupon code. Try WELCOME10, SAVE50, or FREESHIP.');
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
      ...
      {!appliedCoupon ? (
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            <Tag size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-secondary)', pointerEvents: 'none' }} />
            <input
              type="text"
              value={couponInput}
          ...
          <button
            ...
            style={{
              ...
              opacity:         couponInput.trim() && !applying ? 1 : 0.5,
              whiteSpace:      'nowrap',
            }}
          >
            {applying ? '…' : 'Apply'}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'rgba(74,124,44,0.07)', border: '1px solid rgba(74,124,44,0.18)', borderRadius: 'var(--radius-md)', padding: '8px 12px' }}>
          ...
          <button type="button" onClick={handleRemoveCoupon} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-secondary)', display: 'flex', padding: 2 }}>
            <X size={14} />
          </button>
        </div>
      )}
      ...
      {/* Coupon message */}
      {couponMsg && (
        <p style={{ ...smallTextStyle, color: couponStatus === 'error' ? 'var(--color-error)' : 'var(--color-success)', margin: '-8px 0 0' }}>
          {couponMsg}
        </p>
      )}

      {/* Totals */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, borderTop: '1px solid var(--color-border)', paddingTop: 'var(--spacing-md)' }}>
        ...
        <TotalRow label="Shipping"  value={cartShipping === 0 ? 'Free' : fmt(cartShipping)} valueColor={cartShipping === 0 ? 'var(--color-success)' : undefined} />
        <TotalRow label="VAT (16%)" value={fmt(cartTax)} small />
...
function TotalRow({ label, value, bold = false, small = false, valueColor }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <span style={{ fontFamily: 'var(--font-sans)', fontSize: small ? 12 : 14, fontWeight: bold ? 700 : 500, color: valueColor || (bold ? 'var(--color-earth-brown)' : 'var(--color-text-primary)') }}>
        {value}
      </span>
    </div>
  );
