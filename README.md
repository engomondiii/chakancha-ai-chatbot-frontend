...
- Kept truly external `http://` and `https://` image URLs unchanged.
- Added same-origin URL normalization so Chakancha public assets returned by the backend as full URLs, such as `https://chakancha.com/images/...`, are converted back to local Next.js public paths such as `/images/...`.
- Applied the same local-path treatment to `/icons/...` assets.
- Removed accidental `/public` prefixes before rendering frontend public assets.
- Continued resolving Django/Railway media paths against `NEXT_PUBLIC_API_URL`.
...
- Fixed broken PNG gallery thumbnails caused by same-site public assets being treated as remote images by `next/image`.
- Verified that the package, leaves, hot-tea, and iced-tea production asset URLs return HTTP 200 before applying the frontend normalization fix.
...
normalizeProduct()
        ↓
same-site https://chakancha.com/images/... URL
        ↓
local /images/... path
        ↓
...
This separation keeps the catalog visually clean, allows the product-detail page to use the complete gallery, and ensures frontend assets stored under `public/` are rendered as local Next.js assets instead of unnecessary remote-image requests.
...
- Same-site Chakancha image URLs are normalized to local `/images/...` paths so `next/image` renders public assets reliably.
- Broken secondary PNG gallery thumbnails were resolved without changing the Django product-image records.
